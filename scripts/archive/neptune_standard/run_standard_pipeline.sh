#!/usr/bin/env bash
# UnityPSF v0.4 standard end-to-end pipeline submitter.
#
# Default route:
...
#   PIPELINE_MODE=infer_only RUN_DIR=/path/to/run bash scripts/archive/neptune_standard/run_standard_pipeline.sh
#
# Main outputs:
...
TRAIN_SCRIPT="${TRAIN_SCRIPT:-$UNITY_DIR/scripts/archive/neptune_standard/train_standard_3367_hqzmap.sh}"
CHANNEL_INFER_SCRIPT="${CHANNEL_INFER_SCRIPT:-$UNITY_DIR/scripts/infer/standard_channel_infer_recon.sbatch}"
DUAL_SCRIPT="${DUAL_SCRIPT:-$UNITY_DIR/scripts/infer/run_3371_union_raw_ratio_bicolor.sbatch}"
SAMPLE_TIFF="${SAMPLE_TIFF:-${NEPTUNE_V04_RAW_TIFF_PATH:-${NEPTUNE_V03_RAW_TIFF_PATH:-$ROOT/neptune_iwae/test_data/microtube/raw/spool_800mW_30ms_3D_7_1_MMStack_Default.ome.tif}}}"
PIPELINE_TAG="${PIPELINE_TAG:-standard_v04_$(date +%Y%m%d_%H%M%S)}"
...
if [[ "$PIPELINE_MODE" == "train_infer" ]]; then
  ...
  train_job="$(submit_export "" "$TRAIN_SCRIPT" \
    UNITY_V04_ROOT="$ROOT" \
    NEPTUNE_V04_ROOT="$ROOT" \
    NEPTUNE_V04_RAW_TIFF_PATH="$SAMPLE_TIFF" \
    ...
    RUN_TAG="$RUN_TAG")"
  RUN_DIR="$UNITY_DIR/output/$RUN_TAG/${RUN_TAG}_${train_job}"
...
LEFT_OUTPUT_DIR="${LEFT_OUTPUT_DIR:-$UNITY_DIR/output/$LEFT_RUN_NAME}"
RIGHT_OUTPUT_DIR="${RIGHT_OUTPUT_DIR:-$UNITY_DIR/output/$RIGHT_RUN_NAME}"
DUAL_OUTPUT_DIR="${DUAL_OUTPUT_DIR:-$UNITY_DIR/output/$DUAL_RUN_NAME}"
...
cat <<EOF
submitted_unity_psf_standard_pipeline
mode=$PIPELINE_MODE
