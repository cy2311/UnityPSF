...
def enabled() -> bool:
    return str(os.environ.get("NEPTUNE_V03_PROFILE_TIMING", "")).strip().lower() in {"1", "true", "yes", "on"}


def _sync_cuda() -> None:
    if not enabled():
        return
    if str(os.environ.get("NEPTUNE_V03_PROFILE_SYNC_CUDA", "1")).strip().lower() in {"0", "false", "no", "off"}:
        return
