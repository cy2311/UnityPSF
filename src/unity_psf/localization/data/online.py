...
from unity_psf.localization.smlm_targets import LEGACY_IWAE_PXYZ_TARGET_ORDER, V03_PXYZ_TARGET_ORDER
...
def _validate_online_lut_modes(config: OnlineBatchProviderConfig) -> None:
    if (
        ...
        and str(config.simulation_backend).strip().lower() != "native"
    ):
...
def _poisson_camera_readout(
    config: OnlineBatchProviderConfig,
    frame_photons: torch.Tensor,
    *,
    seed: int,
) -> torch.Tensor:
...
def _condition_feature_order_for_config(config: OnlineBatchProviderConfig) -> tuple[str, ...]:
    fields = tuple(str(item) for item in config.condition_fields)
    names = list(fields) if fields else list(
        condition_feature_order(
            include_domain_onehot=False,
            domain_count=0,
        )
    )
    if bool(config.append_domain_onehot):
...
def _condition_vector_for_config(config: OnlineBatchProviderConfig, vector: torch.Tensor) -> torch.Tensor:
    fields = tuple(str(item) for item in config.condition_fields)
    if not fields:
        return _match_feature_dim(vector, feature_dim=_condition_feature_dim(config))
    if len(fields) != _condition_feature_dim(config):
...
def _cached_sequence_condition_origin(
    ...
    *,
    ...
    col = cell % grid_x
    row = cell // grid_x
    mode = _resolve_field_origin_sampling_mode(config)
    x0 = _grid_axis_origin(
        cell_index=col,
        grid_size=grid_x,
        full_size=int(full_w),
        roi_size=roi_w,
        centered=mode == "cell_center_grid",
    )
    y0 = _grid_axis_origin(
        cell_index=row,
        grid_size=grid_y,
        full_size=int(full_h),
        roi_size=roi_h,
...
def _condition_provider(
    ...
    *,
    ...
    if condition_providers:
        ...
        source = _conditioning_profile_source(config) if domain_name is None else "dual_domain_coeff_maps"
        return provider, source, domain_name
    provider = _default_condition_provider(config)
    return provider, _conditioning_profile_source(config), None


def _conditioning_profile_source(config: OnlineBatchProviderConfig) -> str:
...
def _default_condition_provider(config: OnlineBatchProviderConfig) -> FullResZernikeConditioning:
    profile = str(config.conditioning_profile).strip().lower().replace("-", "_")
    ...
    return build_astigmatism_anchor_conditioning_maps(
        width=int(config.width),
        height=int(config.height),
