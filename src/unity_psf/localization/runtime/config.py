...
def _single_channel_online_config(online_cfg: Mapping[str, Any]) -> dict[str, Any]:
    """Resolve domain-conditioned settings for one bound measurement channel."""

    resolved = dict(online_cfg)
    soft_moe = str(online_cfg.get("conditioning_mode", "channels")) == "film" and str(
        online_cfg.get("expert_mode", "")
    ) == "soft_moe"
    feature_dim = _condition_feature_dim(online_cfg)
    append_domain_onehot = _append_domain_onehot(online_cfg, soft_moe=soft_moe)
    resolved["domain_count"] = 1
    resolved["condition_feature_dim"] = feature_dim
    resolved["append_domain_onehot"] = append_domain_onehot
    resolved["condition_dim"] = feature_dim + (1 if append_domain_onehot else 0)
    return resolved


def _is_astigmatism_expert(train_cfg: Mapping[str, Any]) -> bool:
    return _expert_type(train_cfg) == "astigmatism"


def _is_emitter_2d_expert(train_cfg: Mapping[str, Any]) -> bool:
    return _expert_type(train_cfg) == "emitter_2d"


def _expert_type(train_cfg: Mapping[str, Any]) -> str:
    expert_cfg = train_cfg.get("expert")
    if not isinstance(expert_cfg, Mapping):
        return ""
...
def _has_explicit_astigmatism_condition_contract(train_cfg: Mapping[str, Any]) -> bool:
    model_cfg = train_cfg.get("model")
    ...
    if not isinstance(raw_params, Mapping):
        return False
    return "condition_fields" in raw_params or "condition_dim" in raw_params
...
def _resolve_training_optimizer_config(
    ...
    *,
    ...
    if isinstance(configured_optimizer, Mapping):
        ...
        params = dict(
            _mapping(configured_optimizer.get("params", {}), "train.optimizer.params")
        )
...
def _validate_condition_dimensions(online_cfg: Mapping[str, Any], *, soft_moe: bool) -> None:
    if not soft_moe or not _append_domain_onehot(online_cfg, soft_moe=soft_moe):
        return
