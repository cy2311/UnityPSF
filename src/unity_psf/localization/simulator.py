...
from unity_psf.localization.smlm_targets import V03_PXYZ_TARGET_ORDER, V03_PXYZ_TARGET_UNITS
...
from unity_psf.optics.vector_psf import VectorPSFParams, build_vector_psf_context, noll_to_nm, render_vector_psf_bank
...
def simulate_localization_batch(
    ...
    *,
    ...
    if str(config.psf_type).strip().lower() not in {"vector", "empirical_focal"}:
        raise ValueError("localization simulator requires psf_type='vector' or 'empirical_focal'")
...
def _max_emitters_per_sample(config: LocalizationSimulatorConfig) -> int:
    ...
    return max(1, int(math.ceil(target_active + 6.0 * math.sqrt(target_active))))
...
def _sample_counts(config: LocalizationSimulatorConfig, *, batch_size: int, generator: torch.Generator) -> torch.Tensor:
    ...
    return counts.to(dtype=torch.long).clamp_(max=_max_emitters_per_sample(config))
...
class _VectorEmitterRenderer:
    def __init__(self, config: LocalizationSimulatorConfig) -> None:
        ...
        self.pupil_carrier_complex = None
        if config.pupil_carrier_complex is not None:
            carrier = config.pupil_carrier_complex
            if carrier.ndim != 2 or tuple(carrier.shape) != (int(config.npupil), int(config.npupil)):
                raise ValueError("pupil_carrier_complex must match the configured pupil sampling")
            self.pupil_carrier_complex = carrier.to(device=self.device, dtype=torch.complex64).contiguous()
        ...
        *,
        ...
        with time_block("render_vector_patches"):
            patches = render_vector_psf_bank(
                ...
                coeffs_rad,
                z_t * 1e-6,
                pupil_carrier_complex=self.pupil_carrier_complex,
...
class _EmpiricalEmitterRenderer:
    def __init__(self, config: LocalizationSimulatorConfig) -> None:
        if config.empirical_psf_path is None or config.empirical_psf_channel is None:
            raise ValueError("empirical focal simulator requires empirical_psf_path and empirical_psf_channel")
        if config.empirical_psf_focus_index is None:
            raise ValueError("empirical focal simulator requires empirical_psf_focus_index")
        calibration = load_empirical_focal_psf(
            config.empirical_psf_path,
            channel_id=config.empirical_psf_channel,
            focus_index=config.empirical_psf_focus_index,
        )
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.kernel = torch.from_numpy(calibration.kernel).to(device=self.device)
    ...
    def render_many(
        ...
        *,
        ...
        patches, center_x, center_y = self._render_patches(xs=xs, ys=ys, photons=photons)
        return _place_patches(patches, height=height, width=width, center_x=center_x, center_y=center_y)
    ...
    def _render_patches(
        ...
        *,
        ...
        local_x = xs_t - (center_x.to(dtype=torch.float32) + 0.5)
        local_y = ys_t - (center_y.to(dtype=torch.float32) + 0.5)
        patches = self.kernel.expand(xs_t.numel(), -1, -1).clone()
        patches = _fourier_shift_patches(patches, shift_x_px=local_x, shift_y_px=local_y).clamp_min(0.0)
        patches /= patches.sum(dim=(-2, -1), keepdim=True).clamp_min(1e-12)
        return patches * photons_t[:, None, None], center_x, center_y


def _build_emitter_renderer(config: LocalizationSimulatorConfig):
    psf_type = str(config.psf_type).strip().lower()
    if psf_type == "vector":
        return _build_vector_renderer(config)
    if psf_type == "empirical_focal":
        return _EmpiricalEmitterRenderer(config)
    raise ValueError(f"unsupported localization PSF type: {config.psf_type!r}")
