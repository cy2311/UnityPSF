...
from unity_psf.optics import build_named_nat_config, full_roi_coeff_stack_torch

from .contract import PeakBootstrapConfig
from .nat_optimizer import project_gamma_norm_
...
def export_nat_zmap(config: ExportNATZMapConfig, *, peak_config: PeakBootstrapConfig) -> ExportNATZMapResult:
    ...
    if bool(config.export_alternating):
        alternating_stack = _evaluate_coeff_stack(
            payload["alternating"], nat_config=nat_config, max_gamma_norm_nm=peak_config.max_gamma_norm_nm
        )
    ...
    if bool(config.export_approximate):
        approximate_stack = _evaluate_coeff_stack(
            payload["approximate"], nat_config=nat_config, max_gamma_norm_nm=peak_config.max_gamma_norm_nm
        )
...
def _nat_config_from_summary(
    *,
    summary: dict[str, Any],
    peak_config: PeakBootstrapConfig,
    width: int,
    height: int,
):
    cfg = summary.get("config") if isinstance(summary.get("config"), dict) else {}
    return build_named_nat_config(
        str(peak_config.nat_config_kind),
...
def _evaluate_coeff_stack(
    stage: dict[str, Any],
    *,
    nat_config,
    max_gamma_norm_nm: float | None,
) -> np.ndarray:
    ...
    if gamma.numel() != len(nat_config.gammas):
        ...
        gamma = expanded
    project_gamma_norm_(gamma, max_norm_nm=max_gamma_norm_nm)
