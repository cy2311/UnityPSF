...
from unity_psf.optics import build_named_nat_config, evaluate_zernike_from_roi_positions_torch, get_fov_coordinates_torch
from unity_psf.optics.vector_psf import VectorPSFParams, build_vector_psf_context, noll_to_nm, render_vector_psf_bank, render_vector_psf_stack
...
def _fit_alternating_vector(
    ...
    *,
    ...
    z_min_nm, z_max_nm = _local_z_bounds(config)
    init_z = dataset.z_nm.to(device=device, dtype=torch.float32).clamp(z_min_nm, z_max_nm)
    ...
    snapshot = fit_nat_alternating(
        ...
        gamma_lr=2e-3,
        gamma_l2=0.0,
        max_gamma_norm_nm=config.max_gamma_norm_nm,
...
def _build_patch_dataset(*, config: PeakBootstrapConfig, harvest: dict[str, Any]) -> VectorNATPatchDataset:
    ...
    return VectorNATPatchDataset(
        ...
        z_nm=z_nm[selected].to(dtype=torch.float32).clamp(*_local_z_bounds(config)),
...
def _initial_gamma(*, config: PeakBootstrapConfig, nat_config, approximate: VectorNATFitState) -> torch.Tensor:
    ...
    explicit = initial_gamma_from_zernike_coefficients(config, nat_config)
    for mode in config.initial_zernike_coefficients_nm:
        gamma[_constant_gamma_index(nat_config, _parse_mode(mode))] = explicit[
            _constant_gamma_index(nat_config, _parse_mode(mode))
        ]
    if bool(config.vectorfit_astig_gauge):
...
def initial_gamma_from_zernike_coefficients(config: PeakBootstrapConfig, nat_config) -> torch.Tensor:
    gamma = torch.zeros((len(nat_config.gammas),), dtype=torch.float32)
    for raw_mode, raw_value in config.initial_zernike_coefficients_nm.items():
        mode = _parse_mode(raw_mode)
        index = _constant_gamma_index(nat_config, mode)
        component = nat_config.gammas[index].components[0]
        gamma[index] = float(raw_value) / float(component.weight)
    return gamma


def _parse_mode(value: str) -> tuple[int, int]:
    parts = str(value).replace("(", "").replace(")", "").split(",")
    if len(parts) != 2:
        raise ValueError(f"invalid Zernike mode {value!r}; expected 'n,m'")
    return int(parts[0].strip()), int(parts[1].strip())


def _constant_gamma_index(nat_config, mode: tuple[int, int]) -> int:
    for index, gamma in enumerate(nat_config.gammas):
        components = gamma.components
        if (
            len(components) == 1
            and int(components[0].px) == 0
            and int(components[0].py) == 0
            and (int(components[0].n), int(components[0].m)) == mode
        ):
            return int(index)
    raise ValueError(f"Zernike mode {mode!r} has no constant NAT term in {config_name(nat_config)!r}")


def config_name(nat_config) -> str:
    return f"{len(nat_config.aberrations)}-mode/{len(nat_config.gammas)}-gamma"


def _local_z_bounds(config: PeakBootstrapConfig) -> tuple[float, float]:
    z_min_nm, z_max_nm = (float(value) for value in config.local_z_range_nm)
    if z_min_nm > z_max_nm:
        raise ValueError("local_z_range_nm must be ordered from minimum to maximum")
