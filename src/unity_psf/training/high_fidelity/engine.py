...
from unity_psf.gamma_update import (
...
from unity_psf.localization.roi_posterior_sampling import (
...
from unity_psf.roi_library.loc_harvest import (
    LocHarvestConfig,
    _build_inference_frame_proc,
    build_roi_bank_from_loc_harvest,
)
from unity_psf.runtime import ensure_run_layout, profiling, write_run_manifest, write_stage_status
...
def main() -> int:
    ...
    try:
        ...
        condition_store = (
            ...
            else _condition_store_from_runtime_config(runtime_config)
        )
...
def _run_peak_zmap_bootstrap_if_enabled(
    ...
    *,
    ...
    if _is_single_channel_runtime(train_cfg):
        domains = [_single_channel_peak_domain(train_cfg, domains)]
    ...
    for idx, domain_raw in enumerate(domains):
        ...
        result = run_peak_bootstrap_pipeline(
            ...
            config=_peak_bootstrap_config(
                ...
                anchor_profile=(
                    ASTIGMATISM_660NM_ANCHOR_PROFILE
                    if _is_single_astigmatism_runtime(train_cfg)
                    else None
                ),
            ),
        )
...
def _is_single_astigmatism_runtime(train_cfg: Mapping[str, Any]) -> bool:
    ...
    if not isinstance(expert_cfg, Mapping):
        return False
    return str(expert_cfg.get("name", expert_cfg.get("expert_type", ""))).strip().lower() in {
        "astigmatism",
        "astigmatism_expert",
    }


def _is_single_channel_runtime(train_cfg: Mapping[str, Any]) -> bool:
...
def _single_channel_peak_domain(
    train_cfg: Mapping[str, Any],
    domains: list[object],
) -> dict[str, Any]:
    expert_cfg = train_cfg.get("expert")
    expert = expert_cfg if isinstance(expert_cfg, Mapping) else {}
    channel_id = str(expert.get("channel_id", "main"))
    layout_cfg = train_cfg.get("channel_layout")
    channels = (
        layout_cfg.get("channels", layout_cfg.get("measurement_channels"))
        if isinstance(layout_cfg, Mapping)
        else None
    )
    ...
    for item in domains:
        if not isinstance(item, Mapping):
            continue
        if str(item.get("name", "")) == channel_id:
            return {**dict(item), "name": channel_id}
    if len(domains) != 1:
        raise ValueError(
            "single-channel peak bootstrap has no matching domain for "
            f"channel={channel_id!r}; available domains={[str(item.get('name', '')) for item in domains if isinstance(item, Mapping)]!r}"
        )
    selected = domains[0]
    if not isinstance(selected, Mapping):
        raise ValueError("train.real_tiff_wake.domains entries must be mappings")
    return {**dict(selected), "name": channel_id}
...
def _peak_bootstrap_config(
    ...
    *,
    ...
    return PeakBootstrapConfig(
        ...
        vectorfit_astig_anchor_nm=bootstrap_cfg.get(
            "vectorfit_astig_anchor_nm",
...
def _build_domain_roi_bank_gamma_hooks(
    ...
    *,
    ...
    if physical_context is not None:
        splits = _select_single_channel_roi_split(splits, physical_context.channel.channel_id)
    hooks = []
...
def _select_single_channel_roi_split(
