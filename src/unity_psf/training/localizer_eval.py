...
def build_localizer_eval_provider(
    ...
    *,
    ...
    if condition_store is not None:
        return lambda: list(provider(1))
    fixed_batches = tuple(provider(1))
...
def make_legacy_localization_eval_loss(
    ...
    *,
    ...
    eval_loss_state: dict[str, object] = {
        "pred_sets": [],
        "target_sets": [],
        "batch_offset": 0,
        "first_images": None,
    ...
    def eval_loss_fn(model: torch.nn.Module, batch: TrainingBatch) -> torch.Tensor:
        ...
        if eval_loss_state["first_images"] is None:
            eval_loss_state["first_images"] = _model_images(loc_batch.model_input)[0].detach().cpu()
            eval_loss_state["first_output"] = y_out[0].detach().cpu()
    ...
    def reset_eval_metrics() -> None:
        ...
        eval_loss_state["first_images"] = None
        eval_loss_state["first_output"] = None
    ...
    def eval_artifacts() -> dict[str, torch.Tensor]:
        pred_sets = list(eval_loss_state["pred_sets"])
        target_sets = list(eval_loss_state["target_sets"])
        if not pred_sets or not target_sets:
            return {}
        pred = _concat_emitter_sets(pred_sets)
        target = _concat_emitter_sets(target_sets)
        pred_first = pred.batch_index == 0
        target_first = target.batch_index == 0
        ...
        first_pairs = [
            ...
            if int(target.batch_index[target_index].item()) == 0
        ]
        z_values = torch.tensor(
            [float(target.xyz_px_nm[target_index, 2].item()) for _, target_index in first_pairs],
            dtype=torch.float32,
        )
        z_errors = torch.tensor(
            [
                float(pred.xyz_px_nm[pred_index, 2].item())
                - float(target.xyz_px_nm[target_index, 2].item())
                for pred_index, target_index in first_pairs
            ],
            dtype=torch.float32,
        )
        output = eval_loss_state["first_output"]
        if not isinstance(output, torch.Tensor):
            raise RuntimeError("held-out eval did not capture the first model output")
        probability = liteloc_spatial_integration_probability(output[0].unsqueeze(0))[0]
        reconstruction = probability * output[1].clamp_min(0)
        images = eval_loss_state["first_images"]
        if not isinstance(images, torch.Tensor):
            raise RuntimeError("held-out eval did not capture the first model input")
        return {
            "input_images": images,
            "prediction_xy": pred.xyz_px_nm[pred_first, :2],
            "target_xy": target.xyz_px_nm[target_first, :2],
            "z_values": z_values,
            "z_errors": z_errors,
    ...
    eval_loss_fn.eval_artifacts = eval_artifacts
    return eval_loss_fn


def evaluate_localizer_heldout(
    ...
    *,
    ...
    if not batches:
        raise ValueError("held-out eval provider returned no batches")
    reset = getattr(eval_loss_fn, "reset_eval_metrics", None)
    aggregate = getattr(eval_loss_fn, "aggregate_eval_metrics", None)
    artifacts_fn = getattr(eval_loss_fn, "eval_artifacts", None)
    if not all(callable(item) for item in (reset, aggregate, artifacts_fn)):
        raise TypeError("held-out localization loss is missing its aggregation contract")
    reset()
    eval_loss = evaluate(model=model, batches=batches, loss_fn=eval_loss_fn)
    metrics = dict(aggregate())
    metrics.update(
        {
            "eval_loss": float(eval_loss),
            "sample_count": sum(_training_batch_size(batch) for batch in batches),
            "route_count": len(batches),
        }
    )
    return {"metrics": metrics, "artifacts": dict(artifacts_fn())}
...
def _model_images(model_input: object) -> torch.Tensor:
    if torch.is_tensor(model_input):
        return model_input
    if isinstance(model_input, (tuple, list)) and model_input and torch.is_tensor(model_input[0]):
        return model_input[0]
    raise TypeError("Localization eval model_input must contain an image tensor")


def _training_batch_size(batch: TrainingBatch) -> int:
    if not isinstance(batch.inputs, LocalizationTrainBatch):
        raise TypeError("held-out localization eval requires LocalizationTrainBatch")
    return _model_input_batch_size(batch.inputs.model_input)
