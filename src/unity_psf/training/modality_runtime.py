"""Shared runtime construction and formal contract validation for modality training."""
...
from unity_psf.training import ChannelTrainingContext, ModalityChannelStream, ModalityTrainingRuntime, build_runtime_batch_provider, build_runtime_loss, build_trainer_runtime
from unity_psf.localization.training_adapter import LocalizationTrainBatch
from unity_psf.training.modality_joint import ModalityTrainingBatch
from unity_psf.training.localizer_eval import build_localizer_eval_provider, evaluate_localizer_heldout, localizer_eval_route, make_legacy_localization_eval_loss
...
from .joint_config import bind_instance
...
def config_path(joint_path: Path, value: object) -> Path:
...
def shared_runtime_contract(config: Mapping[str, Any]) -> dict[str, Any]:
...
def modality_batches(provider, channel_id: str):
    def batches(epoch: int):
        for training_batch in provider(epoch):
            ...
            if hasattr(training_batch, "targets") and not isinstance(local_batch, LocalizationTrainBatch):
                yield ModalityTrainingBatch(images=local_batch, conditions=None, target=training_batch, channel_id=channel_id)
                continue
            ...
            images, conditions = model_input if isinstance(model_input, tuple) else (model_input, None)
            yield ModalityTrainingBatch(images=images, conditions=conditions, target=training_batch, channel_id=channel_id)

    return batches


def condition_store_provider_overrides(context):
    ...
    def online_train_batch(params: dict[str, object]):
        return build_online_batch_provider(OnlineBatchProviderConfig(**params), condition_store=context.condition_store)

    return {"online_train_batch": online_train_batch}


def physical_state_snapshot(context) -> dict[str, Any]:
...
def audit_formal_runtime_contracts(
    ...
    *,
    ...
    required_channels = {
        PSFModality.DOUBLE_HELIX: {"main"},
        PSFModality.EMITTER_2D: {"left", "right"},
        PSFModality.ASTIGMATISM: {"left", "right"},
    }[parsed_modality]
    if require_complete and set(runtime_configs) != required_channels:
        channel_label = "main" if required_channels == {"main"} else "left and right"
        raise ValueError(
...
def build_modality_runtime(
    ...
    *,
    ...
    for key, spec in entries:
        parsed = JointExpertKey.parse(key)
        resolved_config_path = config_path(joint_path, spec["config"])
        bound_config = bind_instance(load_config(resolved_config_path), key, device=device)
        ...
        contract = shared_runtime_contract(runtime_config)
        ...
        provider_overrides = condition_store_provider_overrides(physical_context)
        if shared_runtime is None:
            ...
            shared_runtime, _ = prepare_instance_runtime(
                shared_runtime,
                runtime_config,
                config_base_dir=resolved_config_path.parent,
            )
        ...
        eval_route = localizer_eval_route(train_cfg, config_base_dir=resolved_config_path.parent)
        if (
        ...
        physical_state, calibration, provenance = channel_metadata(
            runtime_config,
        ...
        channels[parsed.channel_id] = ModalityChannelStream(
            ...
            batches=modality_batches(provider, parsed.channel_id),
            loss_fn=lambda output, batch, fn=from_output: fn(output, batch.target),
            physical_state=physical_state,
            calibration=calibration,
            provenance=provenance,
            snapshot_physical_state=(
                lambda context=physical_context: physical_state_snapshot(context)
            ),
    ...
    if formal_channels:
        ...
        formal_runtime_evidence = audit_formal_runtime_contracts(modality, runtime_configs)
...
__all__ = [
    ...
    "channel_metadata",
    "condition_store_provider_overrides",
    "config_path",
    "modality_batches",
    "modality_groups",
