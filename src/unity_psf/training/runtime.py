...
def build_trainer_runtime(
    ...
    *,
    ...
    batch_provider = build_runtime_batch_provider(
        config,
        batch_provider_registry=batch_provider_registry,
        batch_provider_overrides=batch_provider_overrides,
    )
    loss_fn = build_runtime_loss(config, loss_registry=loss_registry)
...
def build_runtime_batch_provider(
    config: Mapping[str, object],
    *,
    batch_provider_registry: Mapping[str, BatchProviderFactory] | None = None,
    batch_provider_overrides: Mapping[str, BatchProviderFactory] | None = None,
) -> BatchProvider:
    """Build channel-owned data without constructing another model runtime."""
    ...
    return _build_registered(
        _component_spec(config, "batch_provider"),
        registry=registry,
        label="batch_provider",
    )


def build_runtime_loss(
    config: Mapping[str, object],
    *,
    loss_registry: Mapping[str, LossFactory] | None = None,
) -> LossFn | None:
    """Build channel-owned loss state without constructing another optimizer."""

    if "loss" not in config:
        return None
    registry = {**_builtin_loss_registry(), **dict(loss_registry or {})}
    return _build_registered(
        _component_spec(config, "loss"),
        registry=registry,
        label="loss",
    )
...
def _builtin_batch_provider_registry() -> dict[str, BatchProviderFactory]:
    def deterministic_synthetic_online(params: dict[str, object]) -> BatchProvider:
        from unity_psf.localization.synthetic import (
    ...
    def microtube_tiff_train_batch(params: dict[str, object]) -> BatchProvider:
        from unity_psf.localization.microtube_tiff import (
...
def _builtin_loss_registry() -> dict[str, LossFactory]:
    def active_smlm_loss(params: dict[str, object]) -> LossFn:
        from unity_psf.localization.losses import ActiveSMLMLoss
        from unity_psf.localization.training_adapter import make_localization_loss
    ...
    def localization_mse(params: dict[str, object]) -> LossFn:
        from unity_psf.localization.smlm_output import decode_smlm_output
        from unity_psf.localization.training_adapter import make_localization_loss
